...
| `Subagent`        | `subagent_type` or `"Subagent"`    | `subagent_desc`                                | `ClaudeIcon` or `Warning` (if `tool_error`)     | `✦` (U+2726)       |
...
| `ToolCall`        | Two sections: `Input` and `Output`. **Edit tool** input renders as a structural diff (`.detail-item__diff`) with file path header and a "replace all" badge when `replace_all` is true: `parseEditInput()` extracts the strings and `computeEditDiff()` (shared/diff.ts) produces a line-level diff that keeps unchanged **context** lines (`--context`, dim) plus `−` removed / `+` added lines, with intra-line **word-level** changes wrapped in `.detail-item__diff-word` spans (stronger tint). `EditDiffLines` renders it. **Other tools** render input as `<pre><code>{formatJson(tool_input)}</code></pre>`. Output: `tool_result_json` as `<pre><code>` if set, else `formatJson(tool_result)` as `<pre><code>` if valid JSON, else — when `splitDiffSections()` (shared/diff.ts) finds an embedded unified-diff hunk (e.g. a Bash result under `bashEditDiffEnabled`, issue #315) mixed into the plain text — alternating plain-text spans and `RawDiffBlock` diff sections (same `.detail-item__diff-line` context/removed/added styling as the Edit view, plus a `--header` variant for `---`/`+++`/`@@` lines), else plain text; `.detail-item__text--error` if `tool_error` |
| `Subagent`        | Up to 5 labelled sections: `Agent ID` (mono), `Description`, `Prompt`, `Content` (`text`), and (if `tool_result`/`tool_result_json` is set — e.g. a failed Task/subagent call) an `Output`/`Error` section reusing the same `ToolResultBody` renderer as `ToolCall`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
