...
| `file-history-snapshot` entries may be pruned — only the latest backup per file is retained, so sessions with many Edit calls may have fewer snapshots than edits                                                                                                                                                                                                                                                              | v2.1.208+                                                   | `"file-history-snapshot"` was already in `NOISE_ENTRY_TYPES` and dropped before any processing. No chain reconstruction depends on these entries, so gap-tolerance is inherent. A comment in `NOISE_ENTRY_TYPES` and a regression test (`scan_session_metadata_tolerates_pruned_file_history_snapshots`) lock in this behaviour.                                                                                                                                                                                                                                                                                                                                                                                                                       |
