...
| `dirs`          | Every distinct working directory the session touched, in first-seen order. `dirs[0]` is the origin (the folder the JSONL file lives under); `dirs[-1]` equals `cwd`. Drives the project-node label (by origin) and the session-row "worked in" list. Optional — older cached payloads fall back to `cwd`. |
